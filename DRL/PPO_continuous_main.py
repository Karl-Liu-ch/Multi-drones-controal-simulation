...
import argparse
from DRL.normalization import Normalization, RewardScaling
from DRL.replaybuffer import ReplayBuffer
from DRL.ppo_continuous import PPO_continuous
        ...
        while episode_steps < args.max_episode_steps:
            ...
            if done:
                for drone in env.drones:
                    print(np.linalg.norm(env.drones[k].position - env.drones[k].end) for k in range(len(env.drones)))
            if done: break
        print(np.linalg.norm(env.drones[k].position - env.drones[k].end) for k in range(len(env.drones)))
        # if vis == True:
...
def main(args, env, number, seed, num_obs, num_drone):
    # env_evaluate =   # When evaluating the policy, we need to rebuild an environment
    # Set random seed
    # env.seed(seed)
    ...
    while total_steps < args.max_train_steps:
        ...
        while episode_steps < args.max_episode_steps:
            ...
            if total_steps % args.evaluate_freq == 0:
                evaluate_num += 1
                evaluate_reward = evaluate_policy(args, env, agent, state_norm, (evaluate_num % 2) == 0)
...
if __name__ == '__main__':
    ...
    main(args, env, number=1, seed=10, num_obs = 20, num_drone = 5)
